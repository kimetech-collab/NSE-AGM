...
@section('admin_content')
    <div class="p-6 space-y-6">
        <x-admin.page-header title="Edit Venue Item" subtitle="Update this venue page content block." />

        <x-admin.panel class="p-6">
            <form method="POST" action="{{ route('admin.venues.update', $venueItem) }}" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="section" value="{{ old('section', $venueItem->section) }}" class="w-full border border-nse-neutral-300 rounded px-3 py-2" required>
                    ...
                    <div>
                        ...
                        <input type="number" name="sort_order" value="{{ old('sort_order', $venueItem->sort_order) }}" min="0" max="9999" class="w-full border border-nse-neutral-300 rounded px-3 py-2">
                ...
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title', $venueItem->title) }}" class="w-full border border-nse-neutral-300 rounded px-3 py-2" required>
                    @error('title')<p class="text-red-600 text-xs mt-1">{{ $message }}</p>@enderror
                </div>

                <div>
                    <label class="block text-sm font-medium text-nse-neutral-700 mb-1">Content</label>
                    <textarea name="content" rows="4" class="w-full border border-nse-neutral-300 rounded px-3 py-2">{{ old('content', $venueItem->content) }}</textarea>
                ...
                <div>
                    ...
                    <textarea name="meta" rows="5" class="w-full border border-nse-neutral-300 rounded px-3 py-2">{{ old('meta', is_array($venueItem->meta) ? implode("\n", $venueItem->meta) : '') }}</textarea>
                    @error('meta')<p class="text-red-600 text-xs mt-1">{{ $message }}</p>@enderror
                </div>

                <div class="space-y-3">
                    <label class="flex items-center gap-3">
                        <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', $venueItem->is_featured) ? 'checked' : '' }} class="rounded border-nse-neutral-300">
                        <span class="text-sm text-nse-neutral-700">Mark as featured</span>
                    </label>
                    <label class="flex items-center gap-3">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $venueItem->is_active) ? 'checked' : '' }} class="rounded border-nse-neutral-300">
